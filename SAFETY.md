...
- `guard_pages`, available only with the `guard-pages` feature on supported
  Linux, macOS, Windows, and BSD targets outside Miri.
...
allocator. Linux uses raw syscalls; macOS and BSD use system C ABI entry
points; Windows uses Kernel32 virtual memory APIs.
...
- macOS and BSD: system `mmap` creates a private anonymous read/write mapping,
  `mlock` locks it, and `munlock`/`munmap` release it.
...
- Secret bytes are not copied into the mapping until platform setup succeeds:
  Linux dump exclusion, fork exclusion, and `mlock`; macOS/BSD `mlock`;
  Windows `VirtualLock`.
...
- Linux/macOS/BSD: `mmap(PROT_NONE)` creates one private anonymous mapping
  containing a leading guard page, writable data pages, and a trailing guard
  page. `mprotect(PROT_READ | PROT_WRITE)` enables access only for the middle
  data pages.
...
- The writable data length is rounded to a platform page granule. Linux uses
  ...
  `aarch64` to cover 4 KiB, 16 KiB, and 64 KiB kernels. macOS/BSD use
