...
ownership of an existing heap allocation without copying the new bytes. The old
allocation is cleared first, and the new allocation becomes owned by the secret
container, so its full capacity is covered by later clear/drop operations.
